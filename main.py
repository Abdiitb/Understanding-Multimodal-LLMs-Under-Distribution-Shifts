...
SYNTHETIC_SHIFT_ALL_LIST = ["llava_bench_coco"] + [f"llava_bench_coco{v}" for v in SVSHIFT] + [f"llava_bench_coco{t}" for t in STSHIFT] + [f"llava_bench_coco{v}{t}" for v in SVSHIFT for t in STSHIFT]
...
class CLUB(nn.Module):  # CLUB: Mutual Information Contrastive Learning Upper Bound
    '''
        ...
        Arguments:
            x_dim, y_dim :  pip install -e .       the dimensions of samples from X, Y respectively
...
def get_data_hf(rootpath,hf_ds,ds_name,model):
    ...
    resp_model = [json.loads(q)['text'] for q in open(osp.expanduser(resp_model_fn), "r")]
    return list(subset['image']), list(subset['question']), resp_model, list(subset['reference_answer'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_rootpath", type=str, default="data/data")
